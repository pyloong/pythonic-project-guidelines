...
Pylint 是一个在 Python 源代码中查找 bug 和样式问题的工具。它可以发现一些通常由编译器捕获的问题，这些问题通常是针对 C 和 C++ 等动态程度较低的语言的。由于Python的动态特性，一些警告可能是不正确的;然而，虚假的警告应该是相当少见的。

### 1.1.2 优点

捕获经常忽视的错误。例如打错了，使用在声明前使用变量，等等。
...
如果警告不准确，需要禁用警告，一遍其他问题不被掩盖。若要禁止警告，可以设置行级注释：

```python
dict = 'something awful'  # Bad idea... pylint: disable=redefined-builtin
```

`pylint` 警告都是通过富豪命 (empty-docstring) 标识的， Google 规范的警告是以 `g-` 开头的。
...
可以通过删除函数开头的变量来一直未使用的参数的警告。并使用注释解释为什么删除。注释 "Unused" 就行了。例如：
...
只对包和模块使用导入，不对单个类或函数使用导入。注意：类型模块的导入有一个明确的豁免。
...
名称空间管理约定和简单。每个标识符的源以一致的方式表示： `x.Obj` 表示对象 `Obj` 实在模块 `x` 中定义的。

### 1.2.3 缺点

模块名称仍然可能发生冲突。有些模块名称太长。

### 1.2.4 决定

- 使用 `import x` 用于导入软件包和模块
...
异常是大伯代码块的正常控制流以处理错误或其他异常情况的一种方法。

### 1.4.1 优点

正常操作代码的控制流不会受错误错误处理代码的干扰。它还允许控制流在出现某种情况下时跳过多个逻辑。例如，在一个步骤中从 N 个嵌套函数返回，而不必执行通过的错误代码。
...
- 在有问题的地方使用内置异常。例如，抛出一个 `ValureError` 指示变成错误。比如违反了前置条件（需要一个正数，但传递了一个负数）。不要使用 `assert` 语句验证公共 API 的参数值。
  `assert` 用于确保内部正确性，不得强制正确使用，也不表示发生了某些意外事件。如果在后一种情况下需要使用异常，请使用 raise 语句。例如：
...
- 库和包可以定义它们的自己的异常。但这些异常必须从现有的异常类集成。异常名应该以 `Error` 结尾，而且不应引结巴（`foo.FooError`）。
- 禁止捕获所有异常 `expect:` 语句，或者捕获 `Exception` 、 `StandardError` 。Pyhon 在这方面非常宽容， `expect:` 可以捕获所有拼写错误的名称、 `sys.exit()` 调用、 `Ctrl+C` 中断、单元测试失败以及其他各种您根本不想捕获的异常。
    除非：
    - 重新抛出异常，或者
    - 在程序中创建一个隔离点，在这个隔离点钟，异常不会被传播而是被记录或者消除。例如通过保护外币模块来保护线程不崩溃。
- 最小化 `try/except` 块中的代码量。 `try` 的主体越大，这部分代码快中引发代码异常的可能性就越大，而 `try/except` 可能隐藏一个错误。
- 无论 `try` 块中是否引发异常，都使用了 `finally` 子句执行代码。这对于清理（例如关闭文件）通常很有用。
...
虽然它们在技术上是变量，但模块级常量是允许或鼓励的。例如： `MAX_HOLY_HANDGRENADE_COUNT = 3` 。常量必须使用带下划线的全部大写命名。请参阅下面的命名。

如果需要，应在模块级别声明全局变量，并通过在模块名称前加上 `_` 来使其成为模块内部的变量。外部访问必须通过公共模块级函数完成。请参阅下面的命名。
...
无法对嵌套类或内部类进行 pickle 。不能直接测试嵌套的函数和类。嵌套会使你的外部功能变得更长、更难读。

### 1.6.4 决定

可以使用，但有一些限制。避免使用嵌套函数或类，除非要关闭局部值。不要仅仅为了对用户隐藏模块的某个函数而进行嵌套。相反应该在模块级别的名称上加 `_` 前缀，这样可以测它们。
...
`List` ， `Dict` ， 和 `Set` 推导式以及生成器表达式为创建容器类型和迭代器提供了一种简介有效的方法。而不需要使用传统的循环 `map()` 、 `filter()` 或 `lambda` 。

### 1.7.2 优点

简单理解比其他字典、列表和集合创建技术更加清晰和简单。因为避免创建列表，所以生成器表达式可以非常高效。
...
可以用于简单的情况。每个部分必须适合一行：映射表达式、 `for` 子句 、筛选表达式。不允许使用多个子句或筛选表达式。当逻辑更复杂时，使用循环代替。
...
默认的迭代器和运算符简单而有效。它们直接表达操作。不需要额外的方法调用。使用默认运算符的函数是泛型的。它们可以与支持任何该操作的类型一起使用。
...
对支持它们的类型使用默认迭代器和运算符，如列表、字典和文件。内置类型也定义了迭代器方法。与返回列表的方法相比，更推荐使用这些方法，但是在对容器进行迭代时不应该对其进行修改。除非必要，不要使用 `Python 2` 特定的迭代方法 `dict.iter*()` 。
...
生成器函数返回一个迭代器，该迭代器在每次执行 `yield` 语句时生成一个值。生成值之后，生成器函数的运行时状态将会暂停，知道需要下一个值为止。
...
在生成器函数的 docstging 中使用 `Yields` 而不是 `Returns` 。
...
Lambdas 在表达式中定义匿名函数，而不是在语句中定义。它们通常用于为 `map()` 和 `filter()` 等高阶函数定义灰调函数或运算符。
...
对于像惩罚这种常见的操作，使用来自操作符模块的函数而不是使用 Lambda 函数。例如：推荐使用 `operator.mul` 而不是 `lambda x, y: x * y` 。
...
在函数的参数列表的末尾为变量指定值，例如： `def(a, b=0):` 。如果 `foo` 只有一个参数，那么 `b` 被设置为 -，如果调用时有两个参数，则 `b` 的值为第二个参数。

### 1.12.2 优点

通常会有一个函数使用大量默认值，但在极少数情况下，希望覆盖默认值。默认参数值提供了一种简单的方法类实现这一点，而不必须为罕见的异常行为定义大量的函数。由于 Python 不支持重载的方法/函数，默认参数是“伪造”重载行为的一种简单方法。
...
使用简单或轻量的方式访问或者设置方法的地方，采用属性来访问和设置数据。

### 1.13.1 定义

包装方法是一种方式，一个轻量的方式是将获取和设置一个属性作为标准属性访问。

### 1.13.2 优点

通过消除简单属性访问的显式 `get` 和 `set` 方法调用，可读性变得提高。允许懒加载。认为是 Pythonic 维护类接口的方法。在性能方面，当直接变量访问是合理的时候，允许属性绕过需要简单的访问法方法。这也允许将来在不破坏接口的情况加添加访问器方法。
...
在新代码中使用属性来访问或设置通常使用简单、轻量级访问器或设置器方法的数据。使用 `@property` 创建属性。

如果属性本身未被重写，则带有属性的继承可能不明显。因此必须确保间接调用访问器方法，以确保属性（使用[模板方法设计模式](https://en.wikipedia.org/wiki/Template_method_pattern)）调用子类中重写的方法。
...
Python 在布尔上下文中见将某些值计算为 `False` 。一个快速的“经验法则”是，所有“空”值都被认为是 `False` 。因此在布尔上下文中 `0` ， `None` ， `[]` ， `{}` ， `''` 都被计算为 `False` 。
...
- 必须使用 `if foo is None:` 或 `if foo is not None:` 来检查 `None` 值。例如：在测试默认为 `None` 的变量或参数是否被设置为其他值时。另一个值呢能在布尔上下文中为 `False` 值！
- 千万不要使用 `==` 来比较布尔变量和 `Flase` 。使用 `if not x:` 替代。如果需要区分 `False` 和 `None` ，那么将表达式链接起来，比如： `if not x and x is not None:`
...
当有明显优点时，优先使用装饰去。避免使用 `staticmethod` ，有节制地使用 `classmethod`

### 1.17.1 定义

函数和方法的装饰器（又名为 `@` 标注法）。一个常见的装饰器是 `@property` ，用于将普通方法转换为动态计算的属性。但是 装饰器语法也自定义。具体来说，对于某个函数 `my_decorator` ，可以这么用：
...
内部使用这些特性的标准模块和类是可以使用的。（例如： `abc.ABCMeta` 、 `collections.namedtuple` 、 `dataclasses` 、 和 `enum` ）。
...
Python 3 是 Python 语言中的一个重大变化。虽然现在有的代码通常是 Python 1.7 编写的，但是可以通过一些简单的工作让代码更明确表达出在 Python 3 下不需要更改。
...
还有其他 `from __future__` 语句。在你认为何时的时候使用。推荐中没有包括 `unicode_literals` ，因为在 Python 1.7 中会在许多地方引入隐式的默认编码转化。大多数代码最好在必要时显示使用 `b''` 和 `u''` 字节以及 unicode 字符串文本。

**`six` 、 `future` 和 `past` 库**

当项目需要同时支持 Python 2 和 Python 3 时，可以根据自己的需要使用[`six`](https://pypi.org/project/six/) 、 [`future`](https://pypi.org/project/future/) 、 [`past`](https://pypi.org/project/past/) 。这些库的存在是为了让低吗更清晰和简单。
